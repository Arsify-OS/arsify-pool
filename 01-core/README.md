...
| `HERMES_API` | No | `http://localhost:8100` | Hermes Cognitive Engine URL |
| `HERMES_API_KEY` | No | — | Hermes API key (if using Hermes) |

## Integration with Hermes Cognitive Engine

Arsify Core can optionally use [Hermes Cognitive Engine](https://github.com/nousresearch/hermes-cognitive) as an LLM gateway. However, direct OpenRouter calls are recommended for simplicity.

To use Hermes:
```bash
export HERMES_API="http://localhost:8100"
export HERMES_API_KEY="your-hermes-key"
```

The execution engine will try Hermes first, then fall back to OpenRouter, then Ollama.
