...
4. `/root/.hermes/shared_knowledge_pool.db` (hermes native)
...
**Note:** Hermes Cognitive Engine dapat digunakan sebagai primary gateway, tapi saat ini direct OpenRouter lebih reliable karena:
- Hermes `/v1/portsocket` return pipeline metadata, bukan raw LLM text
- Hermes fast path `/chat` requires Ollama (Currently not running)
...
│                                                             │
│  ┌─────────────────────────────────────────────────────┐    │
│  │  Hermes Cognitive Engine :8100 (optional)           │    │
...
- [ ] Integrate dengan Hermes Cognitive Engine (ketika L3/L4 pipeline stabil)
