...
Alur yang benar:
  ...
  2. Call LLM (Hermes :8100 atau OpenRouter)
...
# ── Config ────────────────────────────────────────────────────────────
HERMES_API     = os.getenv("HERMES_API", "http://localhost:8100")
...
# ── LLM Call ─────────────────────────────────────────────────────────
def call_hermes(system: str, prompt: str, timeout: int = 120) -> Optional[str]:
    """Call Hermes Cognitive Engine :8100 /v1/portsocket (cognitive path).
    
    Uses X-API-Key auth header, 'input' field (not 'messages'),
    and polls async result from /v1/result/{task_id}.
    """
    HERMES_KEY = os.getenv("HERMES_API_KEY", "hermes-secret-change-me-in-production")
    full_input = f"{system}\n\n{prompt}"
    try:
        import httpx
        # Submit task to cognitive path
        r = httpx.post(f"{HERMES_API}/v1/portsocket",
            headers={
                "Content-Type": "application/json",
                "X-API-Key": HERMES_KEY,
                "X-Agent-ID": "senator-execution",
            },
            json={"input": full_input[:30000], "mode": "auto"},
            timeout=30)
        if r.status_code != 200:
            print(f"  Hermes submit failed: {r.status_code} {r.text[:200]}", file=sys.stderr)
            return None
        task_data = r.json()
        task_id = task_data.get("task_id")
        if not task_id:
            print(f"  Hermes: no task_id in response", file=sys.stderr)
            return None
        print(f"  Hermes task: {task_id} (polling...)")
        # Poll for result
        for attempt in range(int(timeout / 3)):
            import time
            time.sleep(3)
            pr = httpx.get(f"{HERMES_API}/v1/result/{task_id}",
            ...
            if status == "SUCCESS":
                ...
                print(f"  Hermes task failed: {err}", file=sys.stderr)
                return None
            # else: still PENDING/STARTED, keep polling
        print(f"  Hermes task timed out after {timeout}s", file=sys.stderr)
        return None
...
def run_senator(domain: str, dry_run: bool = False, test_mode: bool = False) -> dict:
    ...
    # Step 2: Call LLM — skip Hermes (L3/L4 pipeline not returning LLM text),
