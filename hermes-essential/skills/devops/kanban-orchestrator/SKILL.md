...
## Checking multi-agent orchestration status

When the user asks "how's the work going?" or "what's the status?" for a multi-agent setup, check these layers:

1. **Container/process health:**
   ```bash
   docker ps --filter "name=hermes" --format "table {{.Names}}\t{{.Status}}\t{{.Ports}}"
   ps aux | grep -E "(orchestrator|hermes)" | grep -v grep
   ```
...
3. **Task queue state** (if file-based coordination):
   ```bash
   ls -lah /path/to/tasks/ /path/to/results/ /path/to/progress/
   cat progress/*_status.json | jq
   ```

4. **Last activity timestamps:**
   ```bash
   ls -lt /path/to/results/ | head -5
   cat .last_telegram_sent | tail -10
   ```
...
Present findings in a structured terminal-friendly format with box-drawing and status emoji (✅ ⚠️ ❌). Include:
- What's running and for how long
- What's completed vs. blocked
- Last activity timestamp
- Known issues with root cause if available
...
**Status checks without loading the skill.** When user asks about multi-agent work status, load this skill first — it has the systematic check pattern. Don't improvise status checks from scratch.
