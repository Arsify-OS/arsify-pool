...
If `auxiliary` tasks (title_generation, vision, compression, session_search) fail with the error "No LLM provider configured for task=<task> provider=auto", the `auto` provider can't find a backend. Either:
1. Set `OPENROUTER_API_KEY` or `GOOGLE_API_KEY` in `~/.hermes/.env`, OR
2. Explicitly configure both provider AND model for each auxiliary task (recommended to avoid `auto` fallback failures):

```bash
# Common failure: title_generation (example from 2026-05-03 session)
hermes config set auxiliary.title_generation.provider openrouter
### Auxiliary models not working
If `auxiliary` tasks (title_generation, vision, compression, session_search) fail silently, the `auto` provider can't find a backend. Either set `OPENROUTER_API_KEY` or `GOOGLE_API_KEY`, or explicitly configure each auxiliary task's provider:
```bash
# Fix title generation (common failure point)
hermes config set auxiliary.title_generation.provider openrouter
...
For VPS disk cleanup steps to optimize for Hermes Agent, see `references/vps-disk-cleanup.md`.

### Voice not workingompression, session_search) fail silently, the `auto` provider can't find a backend. Either set `OPENROUTER_API_KEY` or `GOOGLE_API_KEY`, or explicitly configure each auxiliary task's provider:
```bash
# Configure title generation (used for naming sessions)
### Auxiliary models not working
If `auxiliary` tasks (title_generation, vision, compression, session_search) fail silently, the `auto` provider can't find a backend. Either set `OPENROUTER_API_KEY` or `GOOGLE_API_KEY`, or explicitly configure each auxiliary task's provider:
```bash
# Fix title generation (common failure point)
